...
const AbsentDetails = () => {
  ...
  useEffect(() => {
    try {
      const detailsArray = JSON.parse(localStorage.getItem('absentformsubmission')) || [];
      console.log(detailsArray);
      if (Array.isArray(detailsArray)) {
        setFormDetails(detailsArray);
      } 
      else {
        console.error('absentformsubmission is not an array:', detailsArray);
        // If 'absentformsubmission' is not an array, set an empty array
        localStorage.setItem('absentformsubmission', JSON.stringify([]));
        setFormDetails([]);
      }
    } 
    catch (error) {
      console.error('Error parsing absentformsubmission from localStorage:', error);
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    try {
      if (isEditing) {
        ...
        localStorage.setItem('absentformsubmission', JSON.stringify(updatedDetails));
        setEditIndex(null);
        ...
        localStorage.setItem('absentformsubmission', JSON.stringify(details));
        setFormVisible(false);
      }
      setFormData({
        employeeName: '',
  ...
  const deleterow = (index) => {
    ...
    localStorage.setItem('absentformsubmission',JSON.stringify(deleteDetails));
    ...
    <div style={{marginTop:'8%'}}>
      ...
      <div className='float-end'>
        <button className='btn btn-secondary my-1' style={{marginRight:'200px'}} onClick={formVisibility}>
          {formVisible? 'Add Absents':'Add Absents'}
        </button>
      </div>
      {formVisible && (
        <div className="modal" style={{display:'block'}}>
          <div className="modal-content w-50">
            ...
            <form onSubmit={handleSubmit}  className='m-auto' style={{width:'50%'}}>
              ...
              <label>absentDate</label>
              <input type='date' className='form-control border border-dark'  name='absentDate' value={formData.absentDate} onChange={handleChange} required/><br></br>
      ...
      <div className='mt-4 mx-5'>
        <div >
          <h2 className='mx-5'> Absents List</h2>
          <p className="mx-5">
